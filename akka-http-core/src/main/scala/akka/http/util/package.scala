...
import akka.stream.{ FlowMaterializer, FlattenStrategy, OldTransformer }
...
package object util {
  ...
  private[http] implicit def enhanceTransformer[T, U](transformer: OldTransformer[T, U]): EnhancedTransformer[T, U] =
  ...
  private[http] implicit class EnhancedSource[T](val underlying: Source[T]) {
    def printEvent(marker: String): Source[T] =
      underlying.transform2("transform",
