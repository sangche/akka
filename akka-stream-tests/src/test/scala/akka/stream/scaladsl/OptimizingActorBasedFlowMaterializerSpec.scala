...
class OptimizingActorBasedFlowMaterializerSpec extends AkkaSpec with ImplicitSender {
  ...
  "ActorBasedFlowMaterializer" must {
    ...
    "optimize filter + map" in {
      ...
      val f = Source(1 to 100).transform("identity", () ⇒ FlowOps.identityTransformer).filter(_ % 2 == 0).map(_ * 2).map(identity).take(20).take(10).drop(5).fold(0)(_ + _)
      Await.result(f, 5.seconds) should be((1 to 100).filter(_ % 2 == 0).map(_ * 2).take(10).drop(5).fold(0)(_ + _))
    }
  }
}